...
// N x curr_k threads
__global__ void init_centroids(int N, int k, int d, int curr_k, double *device_centroids, double *x, double *dist_cents) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= N) return;
    double mn_dist = DBL_MAX;
    for(int i=0;i<curr_k;i++){
        double curr_dist = 0;
        for (int j = 0; j < d; j++) {
            double diff = (x(idx, j) - device_centroids(i, j));
            curr_dist += diff * diff;
        }
        if(curr_dist < mn_dist){
            mn_dist = curr_dist;
...
int main() {
    ...
    cudaMallocAsync(&device_x, d * N * sizeof(double), stream[1]);
    ...
    cudaMemcpyAsync(device_x, x, d * N * sizeof(double), cudaMemcpyHostToDevice, stream[1]);
    ...
    cudaError_t err;
    double *dist_cents;
    cudaMalloc(&dist_cents, N * sizeof(double));
    cudaMemset(dist_cents, 0, N * sizeof(double));

    for(int i=1;i<k;i++){
        cudaMemcpy(centroids, device_centroids, k * d * sizeof(double), cudaMemcpyDeviceToHost);
        init_centroids<<<NUM_BLOCKS_PTS, NUM_THREADS_PTS>>>(N, k, d, i, device_centroids, device_x, dist_cents);
        cudaDeviceSynchronize();
        thrust::device_ptr<double> dev_ptr = thrust::device_pointer_cast(dist_cents);
        // Use thrust::max_element
        thrust::device_ptr<double> iter = thrust::max_element(dev_ptr, dev_ptr + N);
        int max_index = iter - dev_ptr;
        int pt = max_index % N;

        for(int j=0;j<d;j++){
            cudaMemcpy(device_centroids + j*k + i, device_x + pt + j*N, sizeof(double), cudaMemcpyDeviceToDevice);
        }
        cudaMemcpy(centroids, device_centroids, k * d * sizeof(double), cudaMemcpyDeviceToHost);
