...
#define assign_centroid(device_x, N, d, k, dist_pref_sum, device_centroids, device_new_nearest_centroid, device_nearest_centroid, device_change, device_count, dist_cents, stream) \
    for(int i=0;i<k;i++){ \
        int NUM_BLOCKS = (N*d + NUM_THREADS_PTS - 1) / NUM_THREADS_PTS; \
        int stream_no = (i % NUM_PARTITIONS); \
        double *curr_dist_pref_sum = dist_pref_sum + stream_no*N*d; \
        make_dist<<<NUM_BLOCKS, NUM_THREADS_PTS, 0, stream[stream_no]>>>(device_x, N, d, k, i, device_centroids, curr_dist_pref_sum); \
        make_dist_sum<<<N, d, d * sizeof(double), stream[stream_no]>>>(N, d, curr_dist_pref_sum); \
        assign_dist_cent<<<NUM_BLOCKS_PTS, NUM_THREADS_PTS, 0, stream[stream_no]>>>(N, k, d, i, curr_dist_pref_sum, dist_cents); \
    } \
    find_centroid<<<NUM_BLOCKS_PTS, NUM_THREADS_PTS,0,0>>>(N, k, d, dist_cents, device_nearest_centroid, device_new_nearest_centroid, device_change, device_count);
...
// launch on threads = N*d
__global__ void make_dist(int *x, int N, int d, int k, int k0, double *device_centroids, double *dist_pref_sum) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx >= N*d) return;
    int d0 = idx / N;
    int n0 = idx % N;
    double diff = (x(n0, d0) - device_centroids(k0, d0)); // here, k is the index of the centroid
    // here, Nxd is better for coalescing(both)
...
// launch on threads = N*d
__global__ void make_dist_sum(int N, int d, double *dist_pref_sum){
    extern __shared__ double dist_pref_shared[];
    int tid = threadIdx.x;
    int n0 = blockIdx.x;
    dist_pref_shared[tid] = dist_pref_sum[tid*N + n0];
    __syncthreads();
    ...
    dist_pref_sum[(d-1)*N + n0] = dist_pref_shared[d-1];
}

// launch on threads = N
__global__ void assign_dist_cent(int N, int k, int d, int k0, double *dist_pref_sum, double *dist_cents){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx >= N) return;
    dist_cents[k0*N + idx] = dist_pref_sum[(d-1)*N + idx];
}
