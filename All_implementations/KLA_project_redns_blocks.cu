...
__global__ void do_prefix_sum_double(double *centroid_acc, int *count_prefix, int N, int k, int d, int off){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx >= N*k*d) return;
    int part = idx / (N*d);

    int curr = ((part - 1) >= 0 ? count_prefix[part - 1] : 0);
    int n = (count_prefix[part] - curr);

    int under_part = (idx % (N*d));
    ...
    int j = under_part % d;
    centroid_acc = centroid_acc + (curr * d);

    if (i >= off) {
        int val = (i & ((off << 1) - 1));
        if (val >= off) {
            centroid_acc[i*d + j] += centroid_acc[(i - val + off - 1) * d + j];
        }
    }
}
...
int main() {
    ...
    while (change) {
        ...
        for (int off = 1; off < 2*N; off*=2){
            do_prefix_sum_double<<<(N*k*d + NUM_THREADS_PTS -1)/NUM_THREADS_PTS, NUM_THREADS_PTS>>>(device_centroids_acc, device_count_prefix, N, k, d, off);
        }
